...
public String addCart(@RequestParam("pid") String pid, Principal p,Model model){
		
		Product product = productDAO.get(pid);
		
		User user = userDAO.getMail(p.getName());
		System.out.println(user.getMobilenumber());
		Cart crt=cartDAO.getByUserandProduct(p.getName(), pid);
		
		
		if(product.getQuantity()>0){
			
			if(cartDAO.itemAlreadyExist(p.getName(), pid, true))
			{
